...
export const getCurrentUser = async(parameters: getCurrentUserParameters={}) => {
    ...
    if(addAddress || removeAddress){
        ...
        if(removeAddress) {
            updatedAddressesArray = oldAddressesArray.filter((oldAddress)=> oldAddress.address !== address.address)
        }
        ...
        if(addAddress) {
            ...
            updatedAddressesArray = oldAddressesArray?.map((oldAddress)=> {
                const newIsDefaultBillingAddress = changeDefaultBillingAddress ? false : oldAddress.isDefaultBillingAddress
                const newIsDefaultShippingAddress = changeDefaultShippingAddress ? false : oldAddress.isDefaultShippingAddress
                
                if(editAddress) {
                    if(oldAddress.address === address.address) return address
                }
    
                return { ...oldAddress, isDefaultBillingAddress : newIsDefaultBillingAddress, isDefaultShippingAddress : newIsDefaultShippingAddress }
            });
            !editAddress && (updatedAddressesArray.length > 0 ? updatedAddressesArray.push(address) : updatedAddressesArray.push({...address, isDefaultBillingAddress : true, isDefaultShippingAddress : true}));
        }
