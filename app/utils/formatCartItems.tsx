...
import { CartItemType, CartItemProductType, FormatedCartItemType, CombinationsType } from "../types";
...
export const formatCartItems = (cartItems : CartItemType[])=> {
    ...
    cartItems.forEach((cartItem)=> {
        ...
        const alreadyCreatedStoreObject = createdStoreInfo.filter((info)=> info.name === storeName)
        ...
        if(storeObjectisAlreadyCreated){
            ...
            const newFormatedCartItem = {
                ...
                selectedCombination : cartItem.selectedCombination as CombinationsType,
                product : cartItem.product
            }
            createdStore.cartItems.push(newFormatedCartItem)
        }
        else {
            const storeObject: FormatedCartItemType = createStoreObject(storeName, storeId);
            const newFormatedCartItem = {
                quantity : cartItem.quantity,
                selectedCombination : cartItem.selectedCombination as CombinationsType,
                product : cartItem.product
            ...
            createdStoreInfo.push({ name : storeName, i : formatedCartItems.length - 1 });
        }
    });
