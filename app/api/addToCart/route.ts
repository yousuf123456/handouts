...
export async function POST(req: Request) {
  try {
    const { selectedCombination, quantity, productId } = await req.json();
    ...
    const alreadyExistingCartItem = cart?.cartItems.filter(
      (cartItem) => productId === cartItem.productId,
    );
    const itemIsInTheCart =
      alreadyExistingCartItem && alreadyExistingCartItem.length > 0;
