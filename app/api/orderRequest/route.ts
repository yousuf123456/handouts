...
const getRequestStatus = (updatedOrderedProducts : OrderedProductType[])=> {
    ...
    if(cancelled && !returnInProcess && !cancellationInProcess) return "Cancelled"
    if(returnInProcess && !cancellationInProcess && !cancelled) return "Return in Process"
    if(cancellationInProcess && !returnInProcess && !cancelled) return "Cancellation in Process"
...
export async function POST(req: Request){
    try{
        ...
        const updatingOrderedProductsOperations = updatedOrderedProducts.map((updatedOrderedProduct)=> {
            ...
            return prisma.orderedProduct.update({
                ...
                data : {
                    ...data,
                    ...( type === "Cancellation" ? {cancellationRequestId : request.id} : {}),
                    ...( type === "Return" ? {returnRequestId : request.id} : {})
                }
            })
        })

        const results = await prisma.$transaction([...updatingOrderedProductsOperations, ...updatingPackagesOperations]);
