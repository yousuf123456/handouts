...
import { Control, Controller, FieldValues, UseFormRegister, UseFormWatch } from 'react-hook-form';
...
export const FormInputs: React.FC<FormInputsProps> = ({
    ...
    const provinces = ["Azad Kashmir", "Balochistān", "Gilgit-Baltistan", "Islamabad", "Khyber Pakhtunkhwa", "Punjab", "Sindh"]
    ...
    useEffect(()=> {
        if(provinceSelected){
            ...
            setCities(citiesList.filter((cityData) => cityData.admin_name.toLowerCase() === provinceSelected.toLowerCase())
    ...
    <div className='mt-6 flex flex-col gap-8'>
        ...
        <div className='w-full flex gap-8'> 
            <div className='w-full flex flex-col gap-1'>
                <p className='text-slate-600 text-xs font-text font-semibold'>Province</p>
                <Controller 
                    control={control}
                    name='province'
                    render={({ field }) => (
            ...
            <div className='w-full flex flex-col gap-1'>
                <p className='text-slate-600 text-xs font-text font-semibold'>{"Type (optional)"}</p>
                <div className='w-full flex gap-3'>
                    <AddressTypeButton 
                        label='Home'
                        onClick={()=> selectedType !== "Home" ? setSelectedType("Home") : setSelectedType("")}
                        isSelected={selectedType === "Home"}
                    />
                    <AddressTypeButton 
                        label='Office'
                        onClick={()=> selectedType !== "Office" ? setSelectedType("Office") : setSelectedType("")}
        ...
        <div className='w-full flex gap-8'> 
            <div className='w-full h-full flex flex-col gap-6'>
                <div className='w-full flex flex-col gap-1'>
                    <p className='text-slate-600 text-xs font-text font-semibold'>City</p>
                    <Controller 
                        control={control}
                        name='city'
                        render={({ field }) => (
                ...
                <div className='w-full flex flex-col gap-1'>
                    <p className='text-slate-600 text-xs font-text font-semibold'>Area</p>
                    <Controller 
                        control={control}
                        name='area'
                        render={({ field }) => (
                            <SelectOptions 
                                ...
                                options={["Karachi", "Lahore", "Islamabad", "Rawalpindi", "Hunza", "Quetta", "Peshawar"]}
            ...
            <div className='w-full flex flex-col gap-1'>
                <p className='text-slate-600 text-xs font-text font-semibold'>Default Settings</p>

                <div className='border-2 rounded-sm p-2'>
                    <FlagSelector
                        flagLabel='Default shipping address'
                        setFlag={setIsDefaultShippingAddress}
                        defaultChecked={editingAddressIsDefaultShippingAddress ? true : false}
                    />

                    <FlagSelector
                        flagLabel='Default billing address'
                        setFlag={setIsDefaultBillingAddress}
                        defaultChecked={editingAddressIsDefaultBillingAddress ? true : false}
                    />
                </div>
            </div>
        </div>
