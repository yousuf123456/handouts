...
const ButtonProps = cva(
  "font-roboto font-medium tracking-wide text-center rounded-sm transition-all active:scale-90",
  {
    variants: {
      variant: {
        default: "bg-themeBlue hover:bg-blue-600 text-white",
        outline:
          "bg-transparent border-2 border-themeBlue text-themeBlue hover:bg-themeBlue hover:text-white",
...
export const Button: React.FC<ButtonProps> = ({
  ...
  return (
    <button
      disabled={Disabled!}
      className={cn(
        ButtonProps({ variant, size, isLoading, Disabled, className }),
      )}
      {...props}
    />
  );
