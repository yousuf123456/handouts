...
export const WriteReviewForm: React.FC<WriteReviewFormProps> = ({
    ...
    const [productRatingValue, setProductRatingValue] = useState<number>(givenReview?.rating || 5);
    const [productRatingHover, setProductRatingHover] = useState<number>(-1);

    const [storeRatingValue, setStoreRatingValue] = useState<number | null>(givenReview?.sellerResponse || 3);
    const [storeRatingHover, setStoreRatingHover] = useState<number>(-1);

    const [review, setReview] = useState(givenReview?.review || "");
    const [sellerReview, setSellerReview] = useState(givenReview?.sellerReview || "");

    const [images, setImages] = useState<string[]>(givenReview?.reviewImages || []);
    ...
    const onSubmit = ()=> {
        ...
        const storeResponseIncrementData = {
            posRatings : !isHistory ? (storeRatingValue === 3 ? 1 : 0) : (previousStoreResponseIncrementData.posRatings + (storeRatingValue === 3 ? 1 : 0)),
            neuRatings : !isHistory ? (storeRatingValue === 2 ? 1 : 0) : (previousStoreResponseIncrementData.neuRatings + (storeRatingValue === 2 ? 1 : 0)),
            negRatings : !isHistory ? (storeRatingValue === 1 ? 1 : 0) : (previousStoreResponseIncrementData.negRatings + (storeRatingValue === 1 ? 1 : 0))
        }
        ...
        const data = {
            ...
            previousProductRating : previousProductRating ? previousProductRating.toString() : previousProductRating,
            ratingToIncrease : isHistory ? (productRatingValue - previousProductRating) : productRatingValue
        }

        axios.post("../../../../api/writeReview", data)
        .then((res)=> {
    ...
    <div className='flex flex-col gap-12'>
        ...
        <div className='pl-[68px]'>
            <LoadingButton 
                ...
                disabled={isLoading} 
                onClick={onSubmit}
                className='w-48 h-11 flex justify-center text-base'
            >
                Submit
            </LoadingButton>
        </div>
